...
double simpson_integral(pointFunc f, double a, double b, int n) {
  ...
  for (int i = 1; i < n; i += 2) { // высчитываем значения функции при чётных и нечётных точках исключая крайние точки
    k1 += f(a + i*h);
    k2 += f(a + (i+1)*h);
  }
  return h/3*(f(a) + 4*k1 + 2*k2); // считаем интеграл по формуле
}
